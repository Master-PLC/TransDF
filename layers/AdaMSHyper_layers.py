...
from layers.Transformer_EncDec import PositionwiseFeedForward
...
class ConvLayer(nn.Module):
    def __init__(self, c_in, window_size):
        ...
        self.downConv = nn.Conv1d(
            in_channels=c_in, out_channels=c_in, kernel_size=window_size, stride=window_size
        )
    ...
    def forward(self, x):
        # x: [B, d_model, L]
        x = self.downConv(x)  # [B, d_model, L // window_size]
        x = self.norm(x)
        x = self.activation(x)
        return x
...
class Bottleneck_Construct(nn.Module):
    ...
    def forward(self, enc_input):
        # enc_input: [B, L, Dx]
        temp_input = self.down(enc_input).permute(0, 2, 1)  # [B, d_model, L]
        all_inputs = []
        all_inputs.append(temp_input.permute(0, 2, 1))
        # calculate feature of each scale
        for i in range(len(self.conv_layers)):
            temp_input = self.conv_layers[i](temp_input)  # [B, d_model, L // window_size]
            all_inputs.append(temp_input.permute(0, 2, 1))
...
class Decoder(nn.Module):
    ...
    def __init__(self, opt, mask):
        ...
        self.layers = nn.ModuleList([
            DecoderLayer(
                opt.d_model, opt.d_inner_hid, opt.n_head, opt.d_k, opt.d_v, dropout=opt.dropout, normalize_before=False
            ),
            DecoderLayer(
                opt.d_model, opt.d_inner_hid, opt.n_head, opt.d_k, opt.d_v, dropout=opt.dropout, normalize_before=False
            )
