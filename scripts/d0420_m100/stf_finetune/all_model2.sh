...
GPUS=(0 1 2 3)
TOTAL_GPUS=${#GPUS[@]}
MAX_JOBS_PER_GPU=$((MAX_JOBS / TOTAL_GPUS))  # 每个 GPU 的理论最大任务数量

get_gpu_job_counts() {
    local terminal_pids=$(jobs -p | tr '\n' '|')
    terminal_pids=${terminal_pids%|}

    local gpu_job_counts=()
    for gpu in ${GPUS[@]}; do
        local gpu_pids=$(nvidia-smi --query-compute-apps=pid -i $gpu --format=csv,noheader)
        local job_count=$(echo "$gpu_pids" | grep -E "^($terminal_pids)$" | wc -l)
        job_count=${job_count:-0}
        gpu_job_counts+=($job_count)
    done
    echo "${gpu_job_counts[@]}"
}

get_gpu_allocation(){
    local gpu_job_counts=($(get_gpu_job_counts))

    for i in "${!GPUS[@]}"; do
        local gpu=${GPUS[$i]}
        local job_count=${gpu_job_counts[$i]}
